...
const MySchedules = () => {
    ...
    const myBookings = mySchedules.filter(myBooking => myBooking.userEmail === user?.email)
    console.log("My Bookings : ",myBookings);
    const myPendingWorks = mySchedules.filter(myPendingWork => myPendingWork.userEmail !== user?.email)
        ...
        <div>
            ...
            <section>
                <h1 className='text-4xl font-kalam my-10'>My Pending Works</h1>
                {
                    ...
                    <div className='grid grid-cols-2 gap-6'>
                    {
                        myPendingWorks.map(myPendingWork=> <MyPendingCard key={myPendingWork._id} myPendingWork={myPendingWork}></MyPendingCard>)
                    }
                </div> : <h1 className='text-2xl text-red-600'>Nothing  Yet </h1>
